function getAllStudents(connection){
  connection.query("SELECT * FROM `all_students`;", function(err,result){
    ...
    for (const row of result) {
      console.log(`ID: ${row.id}, ${row.first_name} ${row.last_name}, tuition fees: ${row.tuition_fees}, date of birth: ${row.date_of_birth}, course: ${row.description}`);
    }
  });  
};
...
function getAllCourses(connection){
  connection.query("SELECT * FROM `all_courses`;", function(err,result){
    ...
    console.log('\nCourses ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n');
    for (const row of result) {
      console.log(`ID: ${row.id}, Courses details: ${row.title} ${row.stream} ${row.type}, start: ${row.start_date} // end: ${row.end_date}`);
    }
  });  
};
...
function getAllAssignments(connection){
  connection.query("SELECT * FROM `all_assignments`;", function(err,result){
    ...
    console.log('\nAssignments ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n');
    for (const row of result) {
      console.log(`ID: ${row.id}, Assignments details: ${row.title}, ${row.description} ::: type: ${row.type}`);
    }
  });  
};
...
function getAllTrainers(connection){
  connection.query("SELECT * FROM `all_trainers`;", function(err,result){
    ...
    console.log('\nTrainers ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n');
    for (const row of result) {
      console.log(`ID: ${row.id},  ${row.first_name} ${row.last_name}, subject: ${row.title}, course: ${row.description}`);
    }
  });  
};
